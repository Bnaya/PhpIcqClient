...
class httphp2icq extends raw_reader
{
	...
	*/
	...
	var $income_packets = array();
	var $income_packets_pointer = 0;

	function add_income_packet() {
	...
	function check_snac_header($snac_fam, $snac_sub)
	{
		...
		if ($byte = $this->fget_word() != $snac_fam)
		{
			...
			$this->add_error("Unexpected SNAC Family({$byte})" . print_r(debug_backtrace(), 1), 0);
			$this->close_connection();
			return false;
		}
		// Proof SNAC sub-fam
	...
	function read_snac_header()
	{
		...
		// read request idea
		$snac_info['idea'] = $this->fget_dword();
...
XML;
	...
	function send_sms($number , $sms) {
		...
		//$this->check_snac_header(0x0015, 0x0003);
		$snac_info = $this->read_snac_header();
		$c = 0;
		while($snac_info['fam'] != 0x0015 && $snac_info['sub'] != 0x0003 && $c < 3) {
		...
		if($c == 3) {
			...
			$this->add_error('cant get the relevant response');
			return false;
		}
